...
class ProfileViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @objc func handleRefresh(_ refreshControl: UIRefreshControl) {
        if let pubkey = profile?.issuer {
            safeGetTransactions(pubKey: pubkey, callback: {
    ...
    override func viewDidLoad() {
        ...
        if let profile = profile {
            ...
            createTransaction.setImage(imv?.resize(width: 18), for: .normal)
            createTransaction.setTitle("transfer_button_label".localized(), for: .normal)
            createTransaction.layer.cornerRadius = 6
...
//            let ctrl = self.navigationController as! FirstViewController
            ...
            safeGetTransactions(pubKey: profile.issuer)
        }
    ...
    func safeGetTransactions(pubKey: String, callback: @escaping (() -> Void) = {}) {
        DispatchQueue.main.async {
            // poll until we have a validated server node or could not find any
            Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { timer in
                // self.printClassAndFunc(info: "@ presentedVC: \(String(describing: self.presentedViewController))")
                if currentNode != "" {
                    timer.invalidate()
                    self.getTransactions(pubKey: pubKey)
                } else if currentNodeSearchAttmpts > nodes.count {
                    timer.invalidate()
    ...
    private func getTransactions(pubKey: String, callback: @escaping (() -> Void) = {}) {
        ...
        let url2 = "\(currentNode)/tx/history/\(pubKey)"

        assert(url == url2)

        printClassAndFunc("url= \(url)")
