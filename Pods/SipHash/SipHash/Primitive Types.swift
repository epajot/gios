...
public extension SipHasher {
    ...
    mutating func append(_ slice: Slice<UnsafeRawBufferPointer>) {
        append(UnsafeRawBufferPointer(rebasing: slice))
    }
...
public extension SipHasher {
    ...
    mutating func append(_ value: Double) {
        ...
        ///
        /// - Requires: `finalize()` hasn't been called on this instance yet.
        mutating func append(_ value: Float80) {
            var data = value.isZero ? 0.0 : value
...
public extension SipHasher {
    ...
    ///
    /// - Requires: `finalize()` hasn't been called on this instance yet.
    mutating func append<Value: Hashable>(_ value: Value?) {
        if let value = value {
            append(1 as UInt8)
            append(value)
        }
